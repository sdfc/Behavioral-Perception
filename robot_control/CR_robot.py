...
class CR_ROBOT:
    def __init__(self, ip, port):
        ...
        self.cam_intrinsics = np.array(
            [380.15350342, 0., 316.6630249, 0., 379.74990845, 239.63673401, 0, 0, 1]).reshape(3, 3)
        # # Load camera pose (from running calibrate.py), intrinsics and depth scale
        # self.cam_pose = np.loadtxt('cam_pose/camera_pose.txt', delimiter=' ')
        # self.cam_depth_scale = np.loadtxt('cam_pose/camera_depth_scale.txt')
    ...
    def MovL(self, x, y, z, a, b, c, SpeedL=10, AccL=10):
        """
        ...
        if self.port == 30003:
            try:
                self.socket_feedback = socket.socket()
                self.socket_feedback.connect((self.ip, self.port))
                string = "MovL({:f},{:f},{:f},{:f},{:f},{:f},{:f},{:f})".format(x, y, z, a, b, c, SpeedL, AccL)
                print(string)
                self.socket_feedback.send(str.encode(string, 'utf-8'))
            except socket.error:
                print("Fail to setup socket connection !", socket.error)
